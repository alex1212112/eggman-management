  ...
  <div class="app-container">
    ...
    >
      ...
      <el-table-column align="center" label="支付渠道" width="95">
        <template slot-scope="scope">
          <el-tag
            :type=" scope.row.pay_code === 'wechatMiniProgram' ? 'success' : scope.row.pay_code === 'aliPay' ? 'primary' : scope.row.pay_code === 'balance' ? 'warning' : '' "
            effect="dark"
          >
            {{ scope.row.pay_code === 'wechatMiniProgram' ? '微信小程序' : scope.row.pay_code === 'aliPay' ? '支付宝' : scope.row.pay_code === 'balance' ? '余额' : '' }}
          </el-tag>
        </template>
      </el-table-column>
...
<script>
import { getList } from '@/api/payOrder'

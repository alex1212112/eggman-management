<template>
  <div class="app-container">
    ...
    <el-dialog
      :title="dialogEmployeeStatus === 'update' ? '编辑用户' : '创建用户'"
      :visible.sync="dialogEmployeeVisible"
    >
      <el-form
        ref="employeeForm"
        :rules="employeeRules"
...
<script>
import { updateDepartmentUser } from '@/api/department'
import { fetchUserList, updateUser, removeUser } from '@/api/user'
...
export default {
  ...
  methods: {
    ...
    handleUpdateEmployee(scope) {
      this.resetEmployeeTemp()
      this.employeeTemp = {
        id: scope.row.id,
        phone: scope.row.phone,
        realName: scope.row.real_name,
        password: scope.row.password
      }
      this.dialogEmployeeStatus = 'update'

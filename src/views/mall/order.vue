  ...
  <div class="app-container">
    ...
    >
      ...
      <el-table-column align="center" label="商品名称" width="95">
        <template slot-scope="scope">
          {{ scope.row.mall_goods_name }}
        </template>
      </el-table-column>
      <el-table-column align="center" label="商品价格" width="95">
        <template slot-scope="scope">
          {{ scope.row.mall_goods_egg_price }}
        </template>
      </el-table-column>
      <el-table-column align="center" label="购买数量" width="95">
        <template slot-scope="scope">
          {{ scope.row.count }}
        </template>
      </el-table-column>
      ...
      <el-table-column label="状态" align="center">
        <template slot-scope="scope">
          <el-tag
            :type="scope.row.status === 2 ? '' : scope.row.status === 3 ? 'warning' : scope.row.status === 4 ? 'success' : scope.row.status === 5 ? 'danger' : 'info'"
            effect="dark"
          >
      ...
      <el-table-column align="center" label="操作" width="250">
        <template slot-scope="scope">
          <div class="operation-buttons">
            ...
            <el-button v-if="scope.row.status === 2" type="primary" size="small" @click="express(scope.row, scope.$index)">发货</el-button>
          </div>
        </template>
      </el-table-column>
    </el-table>
...
<script>
import { getList, putEdit, putInfo } from '@/api/mallOrder'
